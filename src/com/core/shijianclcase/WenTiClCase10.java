...
public class WenTiClCase10 {
	/**
	 * 问题处理测试用例： 市级中心登记-派发镇街和市职能办，市职能办进行反馈，镇街中心进行反馈，义乌中心进行处理，并重新派发，结案。
	 */
	...
	public void qxznbChuLi() {
		...
		dclQuestion.switchPage(dealQuestion);
		...
		// 退出
		dealQuestion.switchPage(frontPage);
	...
	@Test
	public void ywzxChongXinPaiFa() {
		/**
		 * 义乌中心登录，进入待处理问题页面，点击处理，对镇街中心，区县职能办反馈问题过来的问题进行处理,并重新派发
		 */
		page.login(accounts.get(4));
		BrowserUtil.sleep(2);
		// 义乌中心进入页面首页，点击抬头“事件处理”
		...
		String biaoti2 = dclQuestion.findBybiaoti(title);
		assertEquals(title, biaoti2, "查询不成功");
		// 义乌中心登录，待办理问题页面，点击“处理”,进入处理页面，并且重新派发。
		dclQuestion.clickYiWuChuLi();
		dclQuestion.switchPage(dealQuestion);
		String ywchongxinpaifasm= "义乌中心重新派发说明-结案自动化" + CommonRandUtil.getDate() + "-" + CommonRandUtil.getRam();
		dealQuestion.dealYwFkQuestion2(ywchongxinpaifasm);
		// 退出
		dealQuestion.switchPage(frontPage);
		frontPage.clickYiWuTuiChu();
	}
	@Test
	public void qxznbChuLi2(){
		qxznbChuLi();
	}
	@Test
	...
	public void ywzxJieAn() {
		...
		BrowserUtil.sleep(3);
		// 退出
		yclQuestion.switchPage(frontPage);
		frontPage.clickYiWuTuiChu();
	}
